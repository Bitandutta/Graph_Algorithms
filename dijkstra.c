...
void dijkstra(Graph *g, int source){
	...
	int *d = (int*)malloc(v*sizeof(int)); // tells us to pick which of the following vertex
	int *parent = (int*)malloc(v*sizeof(int));
	int *queue = (int*)malloc(v*sizeof(int)); // tells us which vertex is not yet selected
	
	// Initialize Single Source Begin
	for(i=0; i<v; i++){ // line 1 
		d[i] = infinity; // Line 2
		parent[i] = NIL; // Line 3
		queue[i] = 1; // no one included in the mst
	} 
	d[source]=0; // line 4
	// Initialize Single Source End
	...
	while(1){
		...
		// We are checking if any element in queue is 1 or not
		flag=1; 
		for(i=0; i<v; i++){	
			if(queue[i]==1){
				flag=0; // this vertex is not yet included in the MST
				break;
			}
		}
		if(flag) break; // break if flag=1
		// We break the while loop if all vertices are part of MST
		...
		for(i=0; i<v; i++){
			if( d[i] < min && queue[i]==1 ){
				min = d[i];
				u=i; // i is the vertex with minimum key
			}
		}
		queue[u]=0; // U vertex in now included in the MST
		
		printf("%d, ", u);  // line 6 implementation
		
		// line 7 implementation
		temp = (g->list)[u]; // temp points to the link list storing adjacent vertices of u
	
		while(temp){ // now (temp->v) is adjacent to u
		
			if( (queue[temp->v]==1) && ( d[temp->v] > (d[u] + (temp->w)) ) ){
		...
		for(i=0; i<vertex; i++){
			if( (*(adjMat+(u*vertex)+i))>0 && queue[i]==1 && d[i] > d[u]+(*(adjMat+(u*vertex)+i)) ){
				
				parent[i] = u; // parent of i vertex is u vertex
				
				d[i] = d[u] + *(adjMat+(u*vertex)+i); // storing the weight of the edge in key array
			}
		}
...
int main(){
	...
	int choice;
	printf("\n1. Source-provided Dijkstra\n2. No-source Dijkstra\nEnter choice: ");
	scanf("%d", &choice);
